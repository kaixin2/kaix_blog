...
public class MessageController implements CommunityConstant {
    ...
    public String getLetterList(Model model, Page page) {
        ...
        //每一条私信的相关信息都封装在一个map中,再将所有的map都放入一个列表中
    ...
    public String sendLetter(String toName, String content) {
        ...
        User target = userService.findUserByName(toName); //通过发送者用户名从数据库中得到用户数据
        if (target == null) {
            return CommunityUtil.getJSONString(1, "目标用户不存在"); //发送失败
        }

        Message message = new Message();
        message.setFromId(hostHolder.getUser().getId()); //设置发送者ID
        message.setToId(target.getId());//设置目标发送者id
        if (message.getFromId() < message.getToId()) {
            message.setConversationId(message.getFromId() + "_" + message.getToId());
        } else {
            message.setConversationId(message.getToId() + "_" + message.getFromId());
        }
        message.setContent(content); //设置发信内容
