...
import com.kaixin.copy_echo.util.MailClient;
...
public class UserService implements CommunityConstant {
    ...
    public User findUserById(int id) {
        ...
        User user = getCache(id); // 优先从redid缓存中查询数据
        if (user == null) {
            user = initCache(id); //缓存中没有,从mysql中取,并存入redis缓存
        }
        return user;
    }
     ...
     *
     * 存在的问题有,邮箱服务调用失败,但是依然往数据库中插入了注册的用户信息(未激活)
     */
    public Map<String, Object> register(User user) {
        ...
        //System.out.println("确认激活链接:"+url);
        context.setVariable("url", url);
        String content = templateEngine.process("/mail/activation", context);
        mailClient.sendMail(user.getEmail(),"激活 Echo 账号", content); //发送邮件
        return map;
    }
...
//        user.setPassword(CommunityUtil.md5(user.getPassword() + user.getSalt())); // 加盐加密
//        user.setType(0); // 默认普通用户
//        user.setStatus(1); // 验证通过后直接激活为普通用户
//        user.setActivationCode(CommunityUtil.generateUUID()); // 激活码
//        // 随机头像 使用的牛客的在线头像库（用户登录后可以自行修改）
//        user.setHeaderUrl(String.format("http://images.nowcoder.com/head/%dt.png", new Random().nextInt(1000)));
//        user.setCreateTime(new Date()); // 注册时间
...
//    }
    ...
    /*
     *  将之前的注册用户功能中的 用户信息校检何发送邮件功能拆开
     *  检查用户名,密码,邮箱是否为空,或是否已经被注册
     * */
    public Map<String, Object> check_message(User user) {
        Map<String, Object> map = new HashMap<>();
    ...
    public Map<String, Object> sendEmail(User user) {
        ...
        //将用户的信息作成键,激活码做成值,存入redis中
        String activation = CommunityUtil.generateUUID().substring(0, 6); //生成验证码
        String redisKey = RedisKeyUtil.getRegisterKey(user.getUsername(), user.getPassword(), user.getEmail());
        redisTemplate.opsForValue().set(redisKey, activation, 60, TimeUnit.SECONDS);

        // 给注册用户发送激活邮件
        Context context = new Context();
        context.setVariable("email", user.getEmail());
        context.setVariable("activation", activation); //发送到邮件模板上
        // http://localhost:8080/activation/用户id/激活码
        ...
        try {
            ...
            map.put("activationMsg", exception.getMessage());
            redisTemplate.delete(redisKey); //发送失败的话,直接删除键
        }
        return map;
    }
    ...
    //在redis中比对激活码是否正确
    public int activationRedis(User user, String code) {
        String redisKey = RedisKeyUtil.getRegisterKey(user.getUsername(), user.getPassword(), user.getEmail());
        String activation = (String) redisTemplate.opsForValue().get(redisKey);
        if (activation == null) return ACTIVATION_FAILURE;
        return code.equals(activation) ? ACTIVATION_SUCCESS : ACTIVATION_FAILURE;
    ...
    public void logout(String ticket) {
        ...
        loginTicket.setStatus(1); //1表示登录状态失效
        redisTemplate.opsForValue().set(redisKey, loginTicket);
    }
